...
// plusplus fork hook ①：增强层（作用域通配/排除、作用域感知 system 段、模式→预设绑定）
...
function installInjector(ctx, cfg) {
  ...
  ctx.systemPrompt.section({
    name: "preset-plus",
    order: 100,
    // plusplus fork hook ③：作用域感知的 system 段（strictScope=false 时复刻上游全局常驻行为）
    text: (context) => plusplus.systemSectionText(cfg, ctx, core, context),
  });
  ...
  ctx.on("llm/stream", (options, next) => {
    ...
    // plusplus fork hook ④：条目按「当前模式」解析（支持 modeBindings 模式→预设绑定）
    const entries = plusplus.activeEntries(cfg, core, plusplus.modeOfSession(ctx, sessionId));
    const autoModeOn = (() => {
...
export function apply(ctx, config) {
  // plusplus fork hook ②：包装 scopedPresets，使上游全部 includes 判定获得 `*` / 通配 / `!排除` 能力
