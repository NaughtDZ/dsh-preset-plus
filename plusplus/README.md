...
```bash
dsh plugin --profile web add github:NaughtDZ/dsh-preset-plus   # 装完重启 DSH
```

- `origin` = 你的 fork（`NaughtDZ/dsh-preset-plus`），`upstream` = 官方库（`Rain-kl/dsh-preset-plus`）。
- fork 在 GitHub 上的**默认分支就是 `plusplus`**，所以安装 spec 不需要 `#分支名`；
  要装别的分支才写 `github:NaughtDZ/dsh-preset-plus#<分支>`。
...
- 不要往 `main` 合并：默认分支是 `plusplus`，`main` 只当上游镜像用；`main` 一旦带上 fork 改动，
  以后同步就得重写默认分支，得不偿失。
